  ...
  return (
    <>
      <div className="w-full border-gray-300 mt-4 rounded-t-xl">
        ...
        <div className="flex">
          {/* 기본 서비스 */}
          <div className="border border-gray-300 border-y-0 p-2 w-5/12">
            {basicService.detail
              .filter((service) => service.isIncluded)
              .map((service, idx) => (
                <div
                  key={idx}
                  className={`py-1 px-4 h-1/6 flex items-center justify-start ${
                    idx !==
            ...
            <div className="relative py-1 border-t border-gray-200 flex flex-col h-1/6">
              <div className="flex px-4 items-center justify-start h-full">
                부가세
              </div>
            </div>
          </div>
              ...
              <div className="text-left p-2">
                {additionalService.detail.map((service, idx) => (
                  <div key={idx} className="flex flex-row w-full">
                    <div className="text-left w-full px-4 py-1 border-b border-gray-200">
                      {service.name}
                    </div>
                    <div className="text-right w-full font-semibold border-b border-gray-200">
                      {service.isCounsel
                        ? "나라별 상이"
